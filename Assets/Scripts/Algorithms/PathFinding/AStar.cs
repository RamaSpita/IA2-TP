...
public static class AStar 
{
        ...
        )
    {
        ...
        while (!open.IsEmpty)//Todavia haya nodos para chequear
        {
            ...
            if (satisfies(current,target))//Si el nodo cumple la condicion
            {
            ...
            foreach (var childPair in expand(current))
            {
                ...
                open.Enqueue(child, tentativeCost + heuristic(child,target));//Lo agregamos a la cola
            }
    ...
    private static List<Vector3> ConstructPath(Node end, Dictionary<Node, Node> parents)
    {
        ...
        return path.Select(x => x.center).ToList();
    }
}